/*************************************************************************
	> File Name: 08.function_v1.cpp
	> Author: 
	> Mail: 
	> Created Time: Fri 03 May 2024 10:01:59 PM CST
 ************************************************************************/
...
public :
    normal_func(T (*ptr)(ARGS...)) : func(ptr) {}//转换构造
    normal_func(normal_func &obj) : func(obj.func) {}//拷贝构造
    T run(ARGS ...args) override {//运行函数
        return this->func(std::forward<ARGS>(args)...);
    }
    
    Base<T, ARGS...> *getCopy() {//自我拷贝
...
int main() {
    ...
    func1 = std::move(func);//移动构造
    cout << func1(3, 7) << endl;
    func1 = nullptr;
    return 0;//自己实现function模板类
}
